...
Every object in the output array *MUST* strictly use this structure: `{"index": <number>, "content": <string>}`.
Each object may contain only the `index` and `content` fields, and no other fields.
...
Correct behavior: output `{"index": 257, "content": "- 嗯哼    - 你好"}`.
